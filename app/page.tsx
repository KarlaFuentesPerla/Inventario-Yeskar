...
type Product = { id:number; name:string; category:string; quantity:number; cost:number; publicPrice:number; familyPrice:number; status:ProductStatus; saleType?:SaleType; soldPrice?:number; soldAt?:string };
...
const demoProducts:Product[] = [
  {id:1,name:"Vela Lavanda",category:"Velas",quantity:12,cost:4.25,publicPrice:8.50,familyPrice:7,status:"Disponible"},
  {id:2,name:"Jabón de avena",category:"Cuidado",quantity:6,cost:1.75,publicPrice:4,familyPrice:3.25,status:"Reservado"},
  {id:3,name:"Caja regalo",category:"Regalos",quantity:2,cost:9,publicPrice:18,familyPrice:15,status:"Vendido",saleType:"General",soldPrice:18,soldAt:datePlus(-1)},
  {id:4,name:"Aceite de rosas",category:"Cuidado",quantity:4,cost:5.50,publicPrice:11.50,familyPrice:9.50,status:"Disponible"},
];
...
export default function Home(){
  ...
  useEffect(()=>{const raw=localStorage.getItem("tallercito-v2");if(raw){try{const d=JSON.parse(raw);const loaded:Product[]=d.products.map((p:Product)=>({...p,quantity:p.quantity??1}));setProducts(loaded);setDeliveries(d.deliveries);setCategories(d.categories??Array.from(new Set(loaded.map(p=>p.category))))}catch{}}},[]);
  useEffect(()=>{localStorage.setItem("tallercito-v2",JSON.stringify({products,deliveries,categories}))},[products,deliveries,categories]);
  useEffect(()=>{if(!toast)return;const t=setTimeout(()=>setToast(""),2800);return()=>clearTimeout(t)},[toast]);

  const sold=products.filter(p=>p.status==="Vendido");
  const invested=useMemo(()=>sold.reduce((s,p)=>s+p.cost*p.quantity,0),[sold]);
  const revenue=useMemo(()=>sold.reduce((s,p)=>s+(p.soldPrice??0)*p.quantity,0),[sold]);
  ...
  return <main className="app-shell">
    ...
    <div className="content">
      ...
      {tab==="productos"&&<>
        <section className="page-title"><div><h2>Productos</h2><p>{products.reduce((sum,p)=>sum+p.quantity,0)} unidades en {categories.length} categorías.</p></div><button className="action" onClick={()=>{setEditing(null);setModal("product")}}>＋ Agregar</button></section>
    ...
    {modal&&<div className="modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget){setModal(null);setEditing(null)}}}><section className="modal" role="dialog" aria-modal="true"><button className="close" onClick={()=>{setModal(null);setEditing(null)}} aria-label="Cerrar">×</button>{modal==="product"&&<ProductForm product={editing} categories={categories} onSubmit={saveProduct}/>} {modal==="delivery"&&<DeliveryForm date={selectedDate} onSubmit={saveDelivery}/>} {modal==="sell"&&selected&&<SellForm product={selected} onSubmit={confirmSale}/>}</section></div>}{toast&&<div className="toast">✓ {toast}</div>}
  </main>
}

function ProductCard({product:p,onEdit,onStatus}:{product:Product;onEdit:(p:Product)=>void;onStatus:(p:Product,s:ProductStatus)=>void}){const unitGain=(p.status==="Vendido"?p.soldPrice:p.publicPrice)!-p.cost;return <article className={`product-card ${p.status.toLowerCase()}`}><button className="product-main" onClick={()=>onEdit(p)} disabled={p.status==="Vendido"}><div className="product-image">{p.name.charAt(0)}</div><div><span className={`state ${p.status.toLowerCase()}`}>{p.status}</span><h3>{p.name}</h3><p>{p.category}</p></div><span className="quantity"><b>{p.quantity}</b><small>unidades</small></span></button><div className="prices"><span><small>Compra por unidad</small><b>{money(p.cost)}</b></span><span><small>{p.status==="Vendido"?`Venta ${p.saleType}`:"Venta general"}</small><b>{money(p.status==="Vendido"?p.soldPrice!:p.publicPrice)}</b></span><span className="gain"><small>Ganancia total</small><b>{money(unitGain*p.quantity)}</b></span></div>{p.status!=="Vendido"&&<><button className="edit-product" onClick={()=>onEdit(p)}>Editar producto y cantidad</button><div className="status-actions">{p.status==="Disponible"?<><button className="reserve" onClick={()=>onStatus(p,"Reservado")}>Reservar todo</button><button className="sell" onClick={()=>onStatus(p,"Vendido")}>Vender todo</button></>:<><button className="release" onClick={()=>onStatus(p,"Disponible")}>Volver a disponible</button><button className="sell" onClick={()=>onStatus(p,"Vendido")}>Vender todo</button></>}</div></>}{p.status==="Vendido"&&<div className="locked">✓ {p.quantity} unidades vendidas · Este estado ya no se puede cambiar</div>}</article>}
...
function SellForm({product:p,onSubmit}:{product:Product;onSubmit:(e:FormEvent<HTMLFormElement>)=>void}){const [type,setType]=useState<SaleType>("General"),price=type==="General"?p.publicPrice:p.familyPrice;return <form onSubmit={onSubmit}><span className="eyebrow">CONFIRMAR VENTA</span><h2>{p.name}</h2><p className="warning">Venderás las {p.quantity} unidades. Después de confirmar, no se podrá cambiar.</p><fieldset><legend>¿A quién se vendió?</legend><label className={type==="General"?"chosen":""}><input type="radio" name="saleType" value="General" checked={type==="General"} onChange={()=>setType("General")}/><span><b>Público general</b><small>{money(p.publicPrice)} por unidad</small></span></label><label className={type==="Familia"?"chosen":""}><input type="radio" name="saleType" value="Familia" checked={type==="Familia"} onChange={()=>setType("Familia")}/><span><b>Familia</b><small>{money(p.familyPrice)} por unidad</small></span></label></fieldset><div className="sale-summary"><span>Cantidad: <b>{p.quantity} unidades</b></span><span>Costo total: <b>{money(p.cost*p.quantity)}</b></span><span>Recibido total: <b>{money(price*p.quantity)}</b></span><strong>Ganancia total: {money((price-p.cost)*p.quantity)}</strong></div><button className="submit danger">Confirmar venta de todo</button></form>}
