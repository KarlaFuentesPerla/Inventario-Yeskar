...
export default function Home(){
  ...
  const upcoming=deliveries.filter(d=>!d.collectedAt&&d.date>=isoDate(now)).sort((a,b)=>`${a.date}${a.time}`.localeCompare(`${b.date}${b.time}`));
  ...
  const payoutReminders=deliveries.filter(d=>d.mode==="Empresa"&&d.remuneration&&!d.remunerationWithdrawn).map(d=>({...d,dueAt:new Date(new Date(`${d.date}T${d.time}:00-06:00`).getTime()+48*60*60*1000)}));

  function saveProduct(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);const newCategory=String(f.get("newCategory")||"").trim();const category=newCategory||String(f.get("category"));const values={name:String(f.get("name")),category,cost:Number(f.get("cost")),publicPrice:Number(f.get("publicPrice")),familyPrice:Number(f.get("familyPrice"))};if(newCategory&&!categories.includes(newCategory))setCategories(x=>[...x,newCategory].sort());if(editing){const received=Math.max(0,Number(f.get("receivedQuantity"))||0);setProducts(x=>x.map(p=>p.id===editing.id?{...p,...values,quantity:p.quantity+received}:p));setToast(received?`${received} unidad${received===1?"":"es"} agregada${received===1?"":"s"} al inventario`:"Producto actualizado")}else{const quantity=Number(f.get("quantity"));setProducts(x=>[{id:createClientId(),soldQuantity:0,salesRevenue:0,quantity,...values},...x]);setToast("Producto agregado")};setEditing(null);setModal(null)}
  ...
  return <main className="app-shell">
    ...
    <div className="content">
      {tab==="inicio"&&<>
        ...
        {!upcoming.length&&<div className="empty upcoming-empty"><b>No tienes entregas pendientes</b><span>Las entregas realizadas seguirán visibles en el calendario.</span></div>}
      </>}
      {tab==="productos"&&<>
    ...
    {modal&&<div className="modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget){setModal(null);setEditing(null)}}}><section className="modal" role="dialog" aria-modal="true"><button className="close" onClick={()=>{setModal(null);setEditing(null)}} aria-label="Cerrar">×</button>{modal==="product"&&<ProductForm product={editing} available={editing?availableFor(editing):0} reserved={editing?reservedFor(editing.id):0} categories={categories} onSubmit={saveProduct} onDeleteProduct={removeProductFromApp} onDeleteCategory={removeCategoryFromApp}/>} {modal==="delivery"&&<DeliveryForm date={selectedDate} products={products} availableFor={availableFor} onSubmit={saveDelivery}/>} {modal==="movement"&&selected&&<MovementForm product={selected} available={availableFor(selected)} type={movementType} onSubmit={confirmMovement}/>}</section></div>}{toast&&<div className="toast">✓ {toast}</div>}
  </main>
}
...
function DeliveryCard({delivery:d}:{delivery:Delivery}){const realized=Boolean(d.collectedAt);return <article className={`delivery-card${realized?" realized":""}`}><div className="date-block"><b>{d.time}</b><span>{money(d.price)}</span></div><div><div className="delivery-labels"><span className="delivery-mode">{d.mode==="Empresa"?`Envío · ${d.company}`:"Entrega propia"}</span>{realized&&<span className="delivery-status">✓ Realizada</span>}</div><h3>{d.client}</h3><p>{d.details}</p>{d.mode==="Empresa"&&<small>Entregar a empresa: {d.handoffDate}</small>}<small>{d.address}</small>{!realized&&<a href={`tel:${d.phone}`}>Llamar al {d.phone}</a>}{realized&&<small className="completed-at">Cliente recogió el {new Intl.DateTimeFormat("es-SV",{day:"numeric",month:"short",hour:"numeric",minute:"2-digit"}).format(new Date(d.collectedAt!))}</small>}</div></article>}
function ProductForm({product,available,reserved,categories,onSubmit,onDeleteProduct,onDeleteCategory}:{product:Product|null;available:number;reserved:number;categories:string[];onSubmit:(e:FormEvent<HTMLFormElement>)=>void;onDeleteProduct:(p:Product)=>void;onDeleteCategory:(category:string)=>void}){
  ...
  return <form onSubmit={onSubmit}>
    ...
    <div className="form-grid">
      ...
      {product?<><div className="wide stock-summary"><span><b>{available}</b> disponibles</span><span><b>{reserved}</b> reservadas</span><strong>{available+reserved} unidades actuales</strong></div><label className="wide">Producto nuevo que llegó<input name="receivedQuantity" type="number" min="1" step="1" inputMode="numeric" placeholder="Escribe solo la cantidad nueva"/><small className="field-help">Esta cantidad se sumará al inventario. Las unidades actuales no se modifican.</small></label></>:<label className="wide">Cantidad inicial<input required name="quantity" type="number" min="1" step="1" inputMode="numeric" placeholder="¿Cuántas unidades tienes?"/><small className="field-help">Cantidad con la que inicia este producto.</small></label>}
